...
const { blockSlot, releaseSlot } = require('./slot.controller');
...
const createBooking = async (req, res) => {
  try {
    const {
      ...
      slotDate, slotTime,          // optional — time slot booking
      stylistId, stylistName,      // optional — stylist selection
    } = req.body;
    ...
    if (!salon) return res.status(404).json({ success: false, message: 'Salon not found or not accepting bookings.' });

    const queue = await Queue.findOne({ salon: salonId });
    if (!queue) return res.status(404).json({ success: false, message: 'Salon queue not configured.' });
    if (queue.isPaused) return res.status(400).json({ success: false, message: `Queue is paused: ${queue.pauseReason || 'Temporarily unavailable'}` });

    // ── If time slot selected, verify it's available ──────────────────────
    if (stylistId && slotDate && slotTime) {
      const Slot = require('../models/Slot.model');
      const existingSlot = await Slot.findOne({
        salon: salonId, stylist: stylistId, date: slotDate, startTime: slotTime,
      });
    ...
    const existing = await Booking.findOne({
      user: req.user._id, salon: salonId,
      bookingStatus: { $in: ['pending', 'confirmed', 'in_progress'] },
    });
    if (existing) {
      return res.status(400).json({ success: false, message: 'You already have an active booking at this salon.', bookingId: existing._id });
    }

    const services = await Service.find({ _id: { $in: serviceIds }, salon: salonId, isAvailable: true });
    if (!services.length) return res.status(400).json({ success: false, message: 'No valid services found.' });

    const serviceSnapshots = services.map((s) => ({
      service: s._id, name: s.name, price: s.price, duration: s.duration, category: s.category,
    }));
    ...
    const booking = await Booking.create({
      ...
      stylistName:   stylistId ? (stylistName || 'Selected stylist') : 'Any available',
      slotDate:      slotDate  || '',
      slotTime:      slotTime  || '',
    ...
    // ── Block slot if stylist + date + time selected ──────────────────────
    if (stylistId && slotDate && slotTime) {
      await blockSlot({ salonId, stylistId, date: slotDate, startTime: slotTime, bookingId: booking._id });
    }
    ...
    if (paymentMethod === 'cod') {
      ...
      await createNotification({
        userId: req.user._id, type: 'booking_confirmed', title: 'Booking Confirmed! ✅',
        message: `Your booking at ${salon.name} is confirmed. Token: #${booking.tokenNumber}. Queue position: ${queuePosition + 1}.`,
        bookingId: booking._id, salonId,
      });
      if (salon.owner) {
        await createNotification({
          userId: salon.owner, type: 'new_booking', title: 'New Booking Received 📋',
    ...
    res.status(201).json({
      success: true,
      message: paymentMethod === 'cod' ? 'Booking confirmed! Pay at the salon.' : 'Booking created. Complete payment to confirm.',
      data: { ...booking.toObject(), queuePosition, estimatedWait, pricing },
    });
  } catch (err) {
...
const getMyBookings = async (req, res) => {
  try {
    ...
    if (statuses) filter.bookingStatus = { $in: statuses.split(',').map(s => s.trim()) };
    else if (status) filter.bookingStatus = status;
    ...
    res.status(200).json({ success: true, total, page: Number(page), pages: Math.ceil(total / Number(limit)), data: bookings });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
};
...
const getBookingById = async (req, res) => {
  try {
    ...
    if (!isOwner && !isAdmin && !isSalonOwner) return res.status(403).json({ success: false, message: 'Access denied.' });
...
const cancelBooking = async (req, res) => {
  try {
    ...
    await Queue.findOneAndUpdate({ salon: booking.salon._id }, { $pull: { activeBookings: booking._id } });

    await createNotification({
      userId: req.user._id, type: 'booking_cancelled', title: 'Booking Cancelled ❌',
      message: `Your booking at ${booking.salon.name} has been cancelled.`,
      bookingId: booking._id, salonId: booking.salon._id,
    });
    if (booking.salon.owner) {
      await createNotification({
        userId: booking.salon.owner, type: 'booking_cancelled', title: 'Booking Cancelled ❌',
...
const getPricingPreview = async (req, res) => {
  try {
    ...
    if (!salonId || !serviceIds) return res.status(400).json({ success: false, message: 'salonId and serviceIds required.' });
